    ...
    def save_config(cls, config: dict, path: Path) -> bool:
        ...
        try:
            FORCE_QUOTE_KEYS = {
                ...
                "lrc-type", "lrc-format",
                "decrypt-m3u8-port", "get-m3u8-port",
                "album-folder-format", "playlist-folder-format", 
                "song-file-format", "artist-folder-format",
                "explicit-choice", "clean-choice", "apple-master-choice",
                "storefront",
                "convert-format", "ffmpeg-path", "convert-extra-args"
            }
            ...
            with open(path, 'w', encoding='utf-8') as f:
                yaml.dump(processed_config, f, Dumper=CustomDumper, 
...
class DockerService:
    ...
    def _get_cached_download(self, url: str, quality: DownloadQuality,
                              single_song: bool) -> Optional[DownloadResult]:
    ...
    def _record_download_cache(self, url: str, quality: DownloadQuality,
                                single_song: bool, files: List[str],
                                cover: Optional[str]):
        cache = self._load_cache()
    ...
    async def _run_command(self, cmd: List[str], timeout: int = 60, 
                           ...
                           log_prefix: str = "") -> Tuple[int, str, str]:
        """执行命令并返回结果"""
        try:
            ...
            if self.debug_mode:
                ...
                if stdout_str:
                    ...
                    if len(stdout_lines) > self.docker_log_lines:
                        logger.info(f"[DEBUG]{log_prefix} stdout (显示最后 {self.docker_log_lines} 行):")
                ...
                if stderr_str:
                    ...
                    if len(stderr_lines) > self.docker_log_lines:
                        logger.info(f"[DEBUG]{log_prefix} stderr (显示最后 {self.docker_log_lines} 行):")
    ...
    async def get_service_status(self) -> ServiceStatus:
        ...
        code, stdout, _ = await self._run_command(
            ["docker", "ps", "--filter", f"name={self.WRAPPER_CONTAINER_NAME}", 
             "--format", "{{.Names}}"]
        )
    ...
    async def build_downloader_image(self) -> Tuple[bool, str]:
        ...
        code, stdout, stderr = await self._run_command(
            ["docker", "build", "-f", str(dockerfile), 
             "-t", self.DOWNLOADER_IMAGE_NAME, str(self.downloader_path)],
            timeout=900
        )
    ...
    async def start_wrapper(self) -> Tuple[bool, str]:
        ...
        await self._run_command(
            ["docker", "rm", "-f", self.WRAPPER_CONTAINER_NAME]
        )
        ...
        cmd = [
            "docker", "run", "-d",
            "--name", self.WRAPPER_CONTAINER_NAME,
            "-v", f"{rootfs_data}:/app/rootfs/data",
            "-p", f"{self.decrypt_port}:10020",
            "-p", f"{self.m3u8_port}:20020",
            "-e", "args=-H 0.0.0.0",
    ...
    async def download(self, url: str, quality: DownloadQuality = DownloadQuality.ALAC,
        ...
        if code != 0:
            ...
            if "缺少已保存的 Apple Music 凭证" in error_msg or "未找到凭证" in error_msg:
        ...
        self._record_download_cache(url, quality, single_song, downloaded_files, cover_path)
    ...
    async def _run_command_with_realtime_log(self, cmd: List[str], timeout: int = 60,
    ...
    async def _log_wrapper_status(self):
        ...
        code, stdout, _ = await self._run_command(
            ["docker", "ps", "-a", "--filter", f"name={self.WRAPPER_CONTAINER_NAME}",
        ...
        code, stdout, _ = await self._run_command(
            ["docker", "logs", "--tail", str(self.docker_log_lines), self.WRAPPER_CONTAINER_NAME],
            log_prefix=" [Wrapper日志]"
        )
        if stdout:
            logger.info(f"[DEBUG] Wrapper 最近 {self.docker_log_lines} 行日志:\n{stdout}")
            ...
            for root, dirs, files in os.walk(directory):
                ...
                for file in files:
                    ...
                    try:
                        ...
                        age = time.time() - mtime
                        logger.info(f"  {sub_indent}{file} ({size} bytes, {age:.0f}s ago)")
                    except:
                        logger.info(f"  {sub_indent}{file}")
        except Exception as e:
            logger.error(f"[DEBUG] 读取目录失败: {e}")
    
    def _find_recent_files(self, directory: Path, 
                           extensions: tuple = ('.m4a', '.flac', '.mp3', '.opus', '.wav'),
                           max_age_seconds: int = 300) -> List[str]:
        """查找最近下载的文件"""
