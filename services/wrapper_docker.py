"""
Wrapper Docker Auto-Deployment Module

Handles automatic clone, build, and version management for wrapper Docker images.
...
class WrapperDockerManager:
    """
    Manages wrapper Docker deployment with automatic source clone and build.

    Features:
    - Auto-clone wrapper source from GitHub
    - Build within Docker container
    - Version tracking via git commit hash
    - Support for x86_64 architecture only
    """

    GITHUB_REPO = "https://github.com/WorldObservationLog/wrapper"
    GITHUB_PROXY = "https://gh-proxy.com"

    VERSION_FILE = "version.json"
    SOURCE_DIR = "wrapper-src"
    ...
    def __init__(
        ...
        """
        Initialize the wrapper Docker manager.

        Args:
            assets_dir: Directory to store wrapper assets
            use_proxy: Whether to use GitHub proxy for clone
            proxy_url: Custom proxy URL (default: gh-proxy.com)
        """
        self.assets_dir = assets_dir
        self.wrapper_dir = assets_dir / "wrapper"
        self.source_dir = self.wrapper_dir / self.SOURCE_DIR
    ...
    def _ensure_directories(self) -> None:
        ...
        (self.wrapper_dir / "rootfs" / "data").mkdir(parents=True, exist_ok=True)
    ...
    def get_local_version(self) -> Optional[VersionInfo]:
        """Get locally installed wrapper version."""
        version_file = self.wrapper_dir / self.VERSION_FILE
    ...
    def _save_version_info(self, commit_hash: str, branch: str = "master") -> None:
    ...
    async def clone_or_update_source(self) -> Tuple[bool, str]:
        """
        Clone or update wrapper source code.
        ...
        if self.source_dir.exists():
            ...
            if not (self.source_dir / ".git").exists():
                ...
                if code != 0:
                    return False, f"submodule update 失败: {stderr}"

                return True, "源码更新完成"

        # Clone new repo (directory doesn't exist or was removed)
        logger.info("克隆 wrapper 源码...")

        code, _, stderr = await self._run_command(
            ["git", "clone", "--recurse-submodules", clone_url, str(self.source_dir)],
    ...
    def create_dockerfile(self) -> Tuple[bool, str]:
        """
        Create Dockerfile for building wrapper from source.

        Returns:
            Tuple of (success, message)
        """
        # Multi-stage build: compile in builder, run in slim image
        # CMakeLists.txt expects NDK at $HOME/android-ndk-r23b
        # Build outputs:
        #   - main -> rootfs/system/bin/main
        #   - wrapper -> project root (via ExternalProject)
        dockerfile_content = '''FROM debian:bookworm AS builder

# Install build dependencies (per README.md)
...
# Download Android NDK r23b to $HOME (CMakeLists.txt expects $HOME/android-ndk-r23b)
RUN wget -q -O /tmp/android-ndk.zip https://dl.google.com/android/repository/android-ndk-r23b-linux.zip \\
...
# Copy source code
COPY wrapper-src /build/wrapper

# Build wrapper (per README.md: mkdir build && cd build && cmake .. && make)
...
# Copy rootfs from builder (contains Android libs and compiled main binary)
COPY --from=builder /build/wrapper/rootfs /app/rootfs

# Copy wrapper binary from builder (ExternalProject outputs to project root)
COPY --from=builder /build/wrapper/wrapper /app/wrapper
...
'''
        ...
        try:
            dockerfile_path = self.wrapper_dir / "Dockerfile"
    ...
    async def build_image(
        self,
        image_name: str = "apple-music-wrapper",
        tag: str = "latest",
        ...
        # Create Dockerfile
        success, msg = self.create_dockerfile()
        if not success:
            return False, msg
        ...
        logger.info(f"开始构建 Docker 镜像 {image_tag}（首次构建可能需要 10-20 分钟）...")

        try:
            process = await asyncio.create_subprocess_exec(
                "docker", "build",
                "--tag", image_tag,
                str(self.wrapper_dir),
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE
            )
    ...
    async def auto_setup(
        ...
        """
        Automatically setup wrapper: clone source and build image.
        ...
        if progress_callback:
            await progress_callback("正在获取 wrapper 源码...")
    ...
    def get_status(self) -> dict:
        """Get wrapper installation status."""
        local_version = self.get_local_version()
